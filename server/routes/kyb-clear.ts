 ...
 * KYB Clear Router
 * 
 * This is an endpoint that properly deletes KYB responses 
...
import { kybResponses, tasks, TaskStatus } from '@db/schema';
...
/**
 * Clear all KYB responses for a task - with transaction support
...
router.post('/api/kyb/clear/:taskId', requireAuth, async (req, res) => {
  ...
  try {
    ...
    console.log('[KYB Clear] Clearing fields for task', { taskId, userId, companyId });
    ...
    const taskResult = await client.query(
      ...
      [taskId, companyId, 'company_kyb']
    );

    if (taskResult.rows.length === 0) {
      await client.query('ROLLBACK');
      console.warn('[KYB Clear] Task not found or not authorized', { taskId, companyId });
    ...
    console.log('[KYB Clear] Deleted responses', { 
      taskId, 
    ...
    if (updateResult.rows.length > 0) {
      ...
      console.log('[KYB Clear] Updated task status', { 
        taskId, 
    ...
    return res.status(200).json({
      success: true,
      message: 'KYB fields cleared successfully',
      taskId,
      progress: 0,
      status: 'not_started'
    });
    ...
    await client.query('ROLLBACK');
    console.error('[KYB Clear] Error clearing fields:', error);
