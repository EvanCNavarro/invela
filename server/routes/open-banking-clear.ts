/**
 * Open Banking Clear Router
 * 
 * This is an endpoint that properly deletes Open Banking responses 
 * and updates task status using transactions.
 */
...
/**
 * Clear all Open Banking responses for a task - with transaction support
...
router.post('/api/open-banking/clear/:taskId', requireAuth, async (req, res) => {
  ...
  try {
    ...
    console.log('[Open Banking Clear] Clearing fields for task', { taskId, userId, companyId });
    ...
    const taskResult = await client.query(
      ...
      [taskId, companyId, 'open_banking']
    );

    if (taskResult.rows.length === 0) {
      await client.query('ROLLBACK');
      console.warn('[Open Banking Clear] Task not found or not authorized', { taskId, companyId });
    ...
    console.log('[Open Banking Clear] Deleted responses', { 
      taskId, 
    ...
    if (updateResult.rows.length > 0) {
      ...
      console.log('[Open Banking Clear] Updated task status', { 
        taskId, 
    ...
    return res.status(200).json({
      success: true,
      message: 'Open Banking fields cleared successfully',
      taskId
    });
  } catch (error) {
    // Rollback in case of error
    await client.query('ROLLBACK');
    console.error('[Open Banking Clear] Error clearing fields:', error);
