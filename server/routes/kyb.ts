...
router.post('/api/kyb-fields/:taskId/update', requireAuth, async (req, res) => {
  try {
    ...
    logger.info(`[KYB API] Updating task ${taskId} status from '${task.status}' to '${newStatus}', progress from ${task.progress}% to ${progressPercentage}%`);
    ...
    const { broadcastProgressUpdate } = await import('../utils/task-broadcast');
    broadcastProgressUpdate(
      taskId,
      progressPercentage,
      newStatus,
