...
import { pool, executeWithNeonRetry, queryWithNeonRetry, checkDatabaseHealth } from '../db';
...
async function testNeonOptimizations() {
  ...
  try {
    ...
    const simpleResult = await queryWithNeonRetry<any[]>('SELECT 1 as value');
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      const result = await queryWithNeonRetry<any[]>('SELECT pg_sleep(0.5), $1 as iteration', [i]);
      console.log(`   Query ${i} completed: ${JSON.stringify(result)}`);
      await setTimeout(500); // Deliberate delay between queries
    }
    console.log();
    
    // Test a complex operation with transaction
    console.log('4. Testing complex operation with transaction...');
    const txnResult = await executeWithNeonRetry(async (client) => {
      // Start transaction
      ...
      try {
        ...
        const insertResult = await client.query(
          'INSERT INTO neon_test (name) VALUES ($1), ($2) RETURNING id, name', 
          ['Test 1', 'Test 2']
        );
    ...
    try {
      ...
      const failResult = await queryWithNeonRetry('SELECT * FROM non_existent_table');
