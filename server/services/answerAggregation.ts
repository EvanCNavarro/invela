...
export async function aggregateAnswers(answers: Answer[]): Promise<AggregatedAnswer[]> {
  ...
  const answersByField = answers.reduce((acc, answer) => {
  ...
  for (const [field_key, fieldAnswers] of Object.entries(answersByField)) {
    // Sort by confidence
    const sortedAnswers = fieldAnswers.sort((a, b) => b.confidence - a.confidence);
    ...
    for (const answer of sortedAnswers) {
      ...
      similarAnswers.forEach(similar => processedAnswers.add(similar.answer));

      // Calculate aggregate confidence
      const allConfidences = [answer.confidence, ...similarAnswers.map(a => a.confidence)];
      ...
      const sourceDocuments = [
        ...
        ...similarAnswers.map(a => a.source_document)
      ...
      aggregatedAnswers.push({
        ...
        similar_answers: similarAnswers.map(a => a.answer)
      });
    }
  }
