...
/**
 * Generate demo values for KYB form fields
 * 
 * @param companyName - Optional company name to use in generated values
 * @returns A record mapping field IDs to demo values
 */
export function generateDemoKybValues(companyName: string = 'Demo Company'): Record<string, string> {
  logger.info(`Generating KYB demo values for company: ${companyName}`);

  const safeCompanyName = companyName.replace(/[^\w\s]/gi, '');
  
  // Company Profile section fields
  const companyProfile = {
    'legalEntityName': safeCompanyName,
    'registrationNumber': '12-3456789',
    'incorporationDate': '5/12/2010',
    'businessType': 'Corporation',
    'jurisdiction': 'Delaware, United States',
  ...
  // Governance & Leadership section fields
  const governance = {
    'contactEmail': `contact@${safeCompanyName.toLowerCase().replace(/\s+/g, '')}.com`,
    'website': `https://www.${safeCompanyName.toLowerCase().replace(/\s+/g, '')}.com`,
    'licenses': 'Money Transmitter License (US), E-Money Institution License (EU)',
    'goodStanding': 'Yes, in good standing with all regulatory authorities',
    'authorizedSigners': 'Jane Smith (CEO), Michael Johnson (CFO)',
    'governmentOwnership': 'No government ownership or control',
    'ultimateBeneficialOwners': 'Jane Smith (60%), Venture Partners LP (25%), Michael Johnson (15%)',
    'directorsAndOfficers': 'Jane Smith (CEO), Michael Johnson (CFO), Sarah Wilson (CTO), Robert Brown (COO), Lisa Chen (General Counsel)',
    'businessScope': 'Financial technology services including payment processing, risk assessment, and compliance solutions',
    'corporateRegistration': 'Certificate of Incorporation available upon request'
  };
  
  // Financial Profile section fields
  ...
  // Operations & Compliance section fields
  const operations = {
    'investigationsIncidents': 'None in the last 5 years',
    'financialStatements': 'Audited statements available for past 3 fiscal years',
    'operationalPolicies': 'Comprehensive policy framework covering AML, KYC, data protection, and information security',
