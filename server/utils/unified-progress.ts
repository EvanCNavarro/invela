...
export async function calculateAndUpdateTaskProgress(
  ...
  try {
    ...
    console.log(`${LOG_PREFIX} Step 1 Complete: Found task - Title: ${currentTask.title}`, {
      taskId,
      taskType,
      ...
      const result = await db
        ...
        .where(
          and(
            ...
            eq(openBankingResponses.status, COMPLETE_STATUS)
          )
        );
      ...
      const result = await db
        ...
        .where(
          and(
            ...
            eq(ky3pResponses.status, COMPLETE_STATUS)
          )
        );
      completedFields = result[0].count;
      ...
      const result = await db
        ...
        .where(
          and(
            ...
            eq(kybResponses.status, COMPLETE_STATUS)
          )
        );
      completedFields = result[0].count;
    ...
    const hasSubmissionDate = !!currentTask.submission_date;
    const hasSubmittedFlag = !!currentTask.submitted;
