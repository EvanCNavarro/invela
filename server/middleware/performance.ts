...
export function performanceMonitor(req: Request, res: Response, next: NextFunction) {
  ...
  res.on('finish', () => {
    ...
    log(`[Performance] ${req.method} ${req.originalUrl} - ${duration.toFixed(2)}ms - Status: ${res.statusCode}`);
    
    // If request took too long, log a warning
    if (duration > 1000) {
      log(`[Performance] ⚠️ Slow request: ${req.method} ${req.originalUrl} - ${duration.toFixed(2)}ms`);
    }
