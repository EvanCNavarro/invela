// This is a complete version of the fixed handleDemoAutoFill function
// that resolves all syntax issues
...
const handleDemoAutoFill = useCallback(async () => {
  ...
  try {
    ...
    try {
      ...
      if (taskType === 'sp_ky3p_assessment') {
        // This should never be reached as KY3P forms are handled above
      ...
      logger.error('[UniversalForm] Error in non-KY3P auto-fill:', innerError);
      toast({
        variant: "destructive",
        title: "Auto-Fill Failed",
        description: innerError instanceof Error ? innerError.message : "There was an error loading demo data",
      });
      setDemoFillLoading(false);
      return false;
    }
  } catch (error) {
    logger.error('[UniversalForm] Auto-fill outer error:', error);
    toast({
      variant: "destructive",
      title: "Auto-Fill Failed",
